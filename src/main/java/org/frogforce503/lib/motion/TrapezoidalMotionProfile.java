...
public final class TrapezoidalMotionProfile extends MotionProfile {
    ...
    public TrapezoidalMotionProfile(Goal start, Goal end, Constraints constraints) {
        super(start, end);
    ...
    private State direct(State in) {
        return new State(in.time, in.position * direction, in.velocity * direction, in.acceleration * direction);
    }

    private Goal direct(Goal in) {
        return new Goal(in.position * direction, in.velocity * direction);
    }
    ...
    public State calculate(double time) {
        ...
        if (time < endAccelerationTime) {
            ...
            position = start.position
                    + (start.velocity + endAccelerationTime * constraints.maxAcceleration / 2) * endAccelerationTime
                    + constraints.maxVelocity * (time - endAccelerationTime);
