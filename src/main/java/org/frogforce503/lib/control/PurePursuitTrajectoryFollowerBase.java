...
public abstract class PurePursuitTrajectoryFollowerBase<DriveSignalType> extends TrajectoryFollower<DriveSignalType> {
    ...
    static PathSegment getArcFromPointsAndHeading(Vector2 pointA, Rotation2 headingA, Vector2 pointB) {
        ...
        if (headingA.isParallel(abMidpoint.getAngle())) {
            if (headingA.equals(abMidpoint.getAngle())) {
                ...
                // Point A points exactly away from point B so we cannot create an arc or a line
                // to connect the two
                // points. We create a line from point A and hope the heading changes so we can
                // create an actual arc.
                // The chance of this case happening is really low, so this should almost never
                // happen.
        ...
        if (headingA.isParallel(midpointNormal)) {
            ...
            center = new RigidTransform2(pointA, headingA)
                    .intersection(new RigidTransform2(abMidpoint, abMidpoint.getAngle().normal()));
        }
    ...
    static Trajectory.Segment findLookaheadSegment(Trajectory trajectory, RigidTransform2 currentPose,
                                                   double lookaheadDistance, double searchDt, Trajectory.Segment startSegment) {
        ...
        while (actualLookaheadDistance < lookaheadDistance && !atEndOfTrajectory) {
            ...
            // Use the last segment of the trajectory if the search time exceeds of the
    ...
    protected DriveSignalType calculateDriveSignal(RigidTransform2 currentPose, Vector2 velocity,
                                                   double rotationalVelocity, Trajectory trajectory, double time, double dt) {
        double searchDt = DEFAULT_SEARCH_DT;
        ...
        // System.out.println("Pose: (" + currentPose.translation.x + ", " +
        // currentPose.translation.y + ")");
        // System.out.println("graphing");
        // If we are within the search time to the end of the trajectory we will say
